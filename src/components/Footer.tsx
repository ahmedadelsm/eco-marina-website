          ...
          <div className="md:col-span-5">
            ...
            <div className="mt-6 space-y-2 text-sm text-white/70">
              <a href={`mailto:${site.email}`} className="flex items-center gap-2 hover:text-white">
                <Icons.Mail className="h-4 w-4 shrink-0 text-brand-green" /> {site.email}
              </a>
              <a href={`tel:${site.phone.replace(/\s/g, "")}`} className="flex items-center gap-2 hover:text-white">
                <Icons.Phone className="h-4 w-4 shrink-0 text-brand-green" /> {site.phone}
              </a>
              <p className="flex items-center gap-2">
                <Icons.MapPin className="h-4 w-4 shrink-0 text-brand-green" /> {site.office}
              </p>
            </div>
          </div>
